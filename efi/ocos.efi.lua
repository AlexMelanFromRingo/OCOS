...
--
-- Tiny on purpose: only the work that has to happen before any disk
-- code can run lives here. The boot-mode menu, recovery flows and
-- pretty splash all live in /sys/boot.lua, where they have room to
-- breathe.
--
-- Boot flow:
...
--      (just an fs UUID), then to a filesystem scan that prefers disks
--      carrying an /.ocos-version marker.
--   4. Read the chosen kernel path (default /init.lua) and call it.
--   5. On any failure: full-screen panic with the reason and key boot info.
