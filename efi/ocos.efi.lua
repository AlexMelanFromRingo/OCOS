-- OCOS BIOS — minified into the EEPROM (≤ 4096 B code, 256 B data).
--
-- Boot flow:
--   1. Bind GPU + screen so we can show messages.
--   2. eeprom.setLabel("OCOS BIOS") so the inventory tooltip distinguishes
--      this machine from a stock OC computer.
--   3. eeprom.getData() is parsed as a Lua table {fs=,kernel=,mode=}.
--      If absent or invalid, fall back to the legacy plain-string form
--      (just an fs UUID), then to a filesystem scan.
--   4. Prefer a filesystem with an /.ocos-version marker so OCOS-bearing
--      disks win when several are present.
--   5. Read the chosen kernel path (default /init.lua) and call it.
--   6. On any failure: full-screen panic with the reason and key boot info.
--
-- The `mode` field, when set to "safe", exports _OCOS_SAFE = true to the
-- kernel so the boot.lua skips autoloads.
...
local function read_boot_data()
  ...
  -- New form: a Lua table.
  local fn = load("return " .. raw, "=boot.cfg", "t", {})
  if fn then
    local ok, t = pcall(fn)
    if ok and type(t) == "table" then return t end
  end
  -- Legacy plain-string fs UUID.
  return { fs = raw }
end
...
local function score(addr)
  -- Prefer disks that look like OCOS, then any disk with /init.lua.
...
-- Stock OC's getBootAddress / setBootAddress shims (kernel + tools rely on
-- these existing as global functions).
